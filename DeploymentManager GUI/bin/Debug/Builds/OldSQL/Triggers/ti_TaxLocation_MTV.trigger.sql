...
IF OBJECT_ID('dbo.ti_TaxLocation_MTV') IS NOT NULL
  BEGIN
    DROP TRIGGER dbo.ti_TaxLocation_MTV
    IF OBJECT_ID('dbo.ti_TaxLocation_MTV') IS NOT NULL
        PRINT '<<< FAILED DROPPING TRIGGER dbo.ti_TaxLocation_MTV >>>'
    ELSE
        PRINT '<<< DROPPED TRIGGER dbo.ti_TaxLocation_MTV >>>'
  END
go


CREATE trigger ti_TaxLocation_MTV on dbo.TaxLocation AFTER INSERT as
begin

-----------------------------------------------------------------------------------------------------------------------------
-- Name:       ti_TaxLocation_MTV
-- Arguments: 
-- Tables:    
-- Indexes:   
-- SPs:        
-- Overview:   This custom insert trigger will write records to the custom MTV_TaxAudit table to maintain
--             an easily identifiable record of new TaxLocation records
...
-- Populate the #TL temp table with all of the inserted records so that we can attach an Idnty column to each record
...
IF OBJECT_ID('dbo.ti_TaxLocation_MTV') IS NOT NULL
  PRINT '<<< CREATED TRIGGER dbo.ti_TaxLocation_MTV >>>'
ELSE
  PRINT '<<< FAILED CREATING TRIGGER dbo.ti_TaxLocation_MTV >>>'
