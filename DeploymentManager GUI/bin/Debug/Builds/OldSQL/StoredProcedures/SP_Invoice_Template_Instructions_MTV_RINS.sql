/*
*****************************************************************************************************
USE FIND AND REPLACE ON SP_Invoice_Template_Instructions_MTV_RINS WITH YOUR view (NOTE:  sp_ is already set
*****************************************************************************************************
*/

/****** Object:  StoredProcedure [dbo].[SP_Invoice_Template_Instructions_MTV_RINS]    Script Date: DATECREATED ******/
PRINT 'Start Script=SP_Invoice_Template_Instructions_MTV_RINS.sql  Domain=  Time=' + CONVERT(VARCHAR(50), GETDATE(), 109) + ' on ' + @@SERVERNAME+'.'+db_name()
GO

IF  OBJECT_ID(N'[dbo].[SP_Invoice_Template_Instructions_MTV_RINS]') IS NULL
      BEGIN
			EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[SP_Invoice_Template_Instructions_MTV_RINS] AS SELECT 1'
			PRINT '<<< CREATED StoredProcedure SP_Invoice_Template_Instructions_MTV_RINS >>>'
	  END
GO
...
ALTER Procedure [dbo].[SP_Invoice_Template_Instructions_MTV_RINS] @i_SlsInvceHdrID int
As
-----------------------------------------------------------------------------------------------------------------------------
...
BEGIN
	EXECUTE	sp_MotivaBuildStatisticsInsertUpdateSQLScripts 'SP_Invoice_Template_Instructions_MTV_RINS.sql'
	PRINT '<<< ALTERED StoredProcedure SP_Invoice_Template_Instructions_MTV_RINS >>>'
END
ELSE
BEGIN
	PRINT '<<< FAILED CREATE OR ALTER on StoredProcedure SP_Invoice_Template_Instructions_MTV_RINS >>>'
END
