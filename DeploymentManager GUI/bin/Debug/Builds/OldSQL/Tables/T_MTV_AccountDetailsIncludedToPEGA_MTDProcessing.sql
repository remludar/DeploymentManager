/*
*****************************************************************************************************
--USE FIND AND REPLACE ON MTV_AccountDetailsIncludedToPEGA_MTDProcessing WITH YOUR TABLE
*****************************************************************************************************
*/
...
/****** Object:  Table [dbo].[MTV_AccountDetailsIncludedToPEGA_MTDProcessing]    Script Date: 02/11/2013 ******/
SET QUOTED_IDENTIFIER OFF
SET ANSI_NULLS ON

IF  OBJECT_ID(N'[dbo].[MTV_AccountDetailsIncludedToPEGA_MTDProcessing]') IS NOT NULL
BEGIN
    DROP TABLE [dbo].[MTV_AccountDetailsIncludedToPEGA_MTDProcessing]
    PRINT '<<< DROPPED TABLE MTV_AccountDetailsIncludedToPEGA_MTDProcessing >>>'
END
...
CREATE TABLE [dbo].[MTV_AccountDetailsIncludedToPEGA_MTDProcessing] (	[AcctDtlID] [int] NOT NULL,
	...
	CREATE NONCLUSTERED INDEX [AcctDtlSlsInvceHdrID_Idx] ON MTV_AccountDetailsIncludedToPEGA_MTDProcessing ([AcctDtlSlsInvceHdrID] ASC ) 
		...
		CREATE NONCLUSTERED INDEX [AcctDtlSrceID_Idx] ON MTV_AccountDetailsIncludedToPEGA_MTDProcessing ([AcctDtlSrceID] ASC, [AcctDtlSrceTble]) 
			WITH (STATISTICS_NORECOMPUTE  = OFF, SORT_IN_TEMPDB = OFF, IGNORE_DUP_KEY = OFF, DROP_EXISTING = OFF, ONLINE = OFF, ALLOW_ROW_LOCKS  = ON
			, ALLOW_PAGE_LOCKS  = ON) ON [PRIMARY]
...
IF  OBJECT_ID(N'[dbo].[MTV_AccountDetailsIncludedToPEGA_MTDProcessing]') IS NOT NULL
  BEGIN
	EXECUTE	sp_MotivaBuildStatisticsInsertUpdateSQLScripts 't_MTV_AccountDetailsIncludedToPEGA_MTDProcessing.sql'
    PRINT '<<< CREATED TABLE MTV_AccountDetailsIncludedToPEGA_MTDProcessing >>>'
  END
ELSE
	 PRINT '<<< FAILED CREATING TABLE MTV_AccountDetailsIncludedToPEGA_MTDProcessing >>>'
