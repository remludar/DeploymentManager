...
IF OBJECT_ID('dbo.td_TaxLocation_MTV') IS NOT NULL
  BEGIN
    DROP TRIGGER dbo.td_TaxLocation_MTV
    IF OBJECT_ID('dbo.td_TaxLocation_MTV') IS NOT NULL
        PRINT '<<< FAILED DROPPING TRIGGER dbo.td_TaxLocation_MTV >>>'
    ELSE
        PRINT '<<< DROPPED TRIGGER dbo.td_TaxLocation_MTV >>>'
  END
go


CREATE trigger td_TaxLocation_MTV on dbo.TaxLocation AFTER DELETE as
begin

-----------------------------------------------------------------------------------------------------------------------------
-- Name:       td_TaxLocation_MTV
-- Arguments: 
-- Tables:    
-- Indexes:   
-- SPs:        
-- Overview:   This custom delete trigger will write records to the custom MTV_TaxAudit table to maintain
--             an easily identifiable record of deleted TaxLocation records
...
-- Populate the #TL temp table with all of the deleted records so that we can attach an Idnty column to each record
...
IF OBJECT_ID('dbo.td_TaxLocation_MTV') IS NOT NULL
  PRINT '<<< CREATED TRIGGER dbo.td_TaxLocation_MTV >>>'
ELSE
  PRINT '<<< FAILED CREATING TRIGGER dbo.td_TaxLocation_MTV >>>'
