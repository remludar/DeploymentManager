...
IF OBJECT_ID('dbo.td_TaxRuleSetRule_MTV') IS NOT NULL
  BEGIN
    DROP TRIGGER dbo.td_TaxRuleSetRule_MTV
    IF OBJECT_ID('dbo.td_TaxRuleSetRule_MTV') IS NOT NULL
        PRINT '<<< FAILED DROPPING TRIGGER dbo.td_TaxRuleSetRule_MTV >>>'
    ELSE
        PRINT '<<< DROPPED TRIGGER dbo.td_TaxRuleSetRule_MTV >>>'
  END
go


CREATE trigger td_TaxRuleSetRule_MTV on dbo.TaxRuleSetRule AFTER DELETE as
begin

-----------------------------------------------------------------------------------------------------------------------------
-- Name:       td_TaxRuleSetRule_MTV
-- Arguments: 
-- Tables:    
-- Indexes:   
-- SPs:        
-- Overview:   This custom delete trigger will write records to the custom MTV_TaxAudit table to maintain
--             an easily identifiable record of deleted TaxRuleSetRule records
...
-- Populate the #TRSR temp table with all of the deleted records so that we can attach an Idnty column to each record
...
While @i_Idnty IS NOT NULL
  Begin
    ...
    -- Get the old RuleValue
    Select @vc_OldRuleValue = IsNull(#TRSR.RuleValue,'')
    From   #TRSR
    Where  #TRSR.Idnty = @i_Idnty
    ...
    if @vc_TaxRule = 'Movement Rule'
      Begin
        -- Strip out quotes from the old RuleValue
...
IF OBJECT_ID('dbo.td_TaxRuleSetRule_MTV') IS NOT NULL
  PRINT '<<< CREATED TRIGGER dbo.td_TaxRuleSetRule_MTV >>>'
ELSE
  PRINT '<<< FAILED CREATING TRIGGER dbo.td_TaxRuleSetRule_MTV >>>'
