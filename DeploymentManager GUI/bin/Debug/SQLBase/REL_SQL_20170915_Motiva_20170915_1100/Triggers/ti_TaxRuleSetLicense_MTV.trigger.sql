...
IF OBJECT_ID('dbo.ti_TaxRuleSetLicense_MTV') IS NOT NULL
  BEGIN
    DROP TRIGGER dbo.ti_TaxRuleSetLicense_MTV
    IF OBJECT_ID('dbo.ti_TaxRuleSetLicense_MTV') IS NOT NULL
        PRINT '<<< FAILED DROPPING TRIGGER dbo.ti_TaxRuleSetLicense_MTV >>>'
    ELSE
        PRINT '<<< DROPPED TRIGGER dbo.ti_TaxRuleSetLicense_MTV >>>'
  END
go


CREATE trigger ti_TaxRuleSetLicense_MTV on dbo.TaxRuleSetLicense AFTER INSERT as
begin

-----------------------------------------------------------------------------------------------------------------------------
-- Name:       ti_TaxRuleSetLicense_MTV
-- Arguments: 
-- Tables:    
-- Indexes:   
-- SPs:        
-- Overview:   This custom insert trigger will write records to the custom MTV_TaxAudit table to maintain
--             an easily identifiable record of new TaxRuleSetLicense records
...
-- Populate the #TRSL temp table with all of the inserted records so that we can attach an Idnty column to each record
...
IF OBJECT_ID('dbo.ti_TaxRuleSetLicense_MTV') IS NOT NULL
  PRINT '<<< CREATED TRIGGER dbo.ti_TaxRuleSetLicense_MTV >>>'
ELSE
  PRINT '<<< FAILED CREATING TRIGGER dbo.ti_TaxRuleSetLicense_MTV >>>'
