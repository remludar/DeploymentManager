...
Select	DISTINCT
		...
		Product.PrdctAbbv																as ProductName,
		...
		'GAL'																			as UOM,
		Case	when ISD.BeginningVolume <> 0
				then ISD.BeginningValue / ISD.BeginningVolume
				else 0.0 end															as PerUnitValue,
		ISD.BeginningValue																as EndingInvValue,
		...
		GCFTAProduct.GnrlCnfgMulti														as FTAProductCode,
		RTRIM(TaxCommodity.Name)														as RATaxCommodity,
		ICSub.ChemFTA																	as RegradeProductFTACode,
		ICSub.ChemTaxCommodity															as RegradeProductTaxCommodity,
		0.0																				as TotalReceipts,
		0.0																				as TotalDisbursement,
		0.0																				as TotalBlend,
		0.0																				as GainLoss,
		NULL																			as BookAdjReason,
		EndInv.EndngInvntryExtrnlBlnce													as EndingPhysicalBalance,
		GCProductGrade.GnrlCnfgMulti													as GradeOfProduct,
		@sdt_InsertDate																	as StagingTableLoadDate,
		'N'																				as Status
-- select *
From	InventorySubledgerDetail ISD with (NoLock)
		...
		Left Outer Join CommoditySubGroup Subgroup with (NoLock)
			on	Subgroup.CmmdtySbGrpID				= IsNull(Product.CmmdtySbGrpID, -1)
		Left Outer Join CommoditySubGroup TaxCommodity with (NoLock)
			on	TaxCommodity.CmmdtySbGrpID			= IsNull(Product.TaxCmmdtySbGrpID, -1)
		...
		Left Outer Join GeneralConfiguration GCProductGrade with (NoLock)
			on	GCProductGrade.GnrlCnfgTblNme		= 'Product'
			and	GCProductGrade.GnrlCnfgQlfr			= 'ProductGrade'
			and	GCProductGrade.GnrlCnfgHdrID		= ISD.DlDtlChmclChmclChldPrdctID
...
From	InventoryChange with (NoLock)
		...
		Left Outer Join CommoditySubGroup TaxCommodity with (NoLock)
			on	TaxCommodity.CmmdtySbGrpID			= IsNull(Product.TaxCmmdtySbGrpID, -1)
		Inner Join DealDetailStrategy with (NoLock)
			on	DealDetailStrategy.DlHdrID			= DealDetail.DlDtlDlHdrID
			and	DealDetailStrategy.DlDtlID			= DealDetail.DlDtlID
			and InventoryChange.MovementDate		between DealDetailStrategy.FromDate and DealDetailStrategy.ToDate
		Inner Join StrategyHeader with (NoLock)
			on	StrategyHeader.StrtgyID				= DealDetailStrategy.StrtgyID
		Left Outer Join GeneralConfiguration GCProfitCtr with (NoLock)
			on	GCProfitCtr.GnrlCnfgTblNme			= 'StrategyHeader'
			and	GCProfitCtr.GnrlCnfgQlfr			= 'ProfitCenter'
			and	GCProfitCtr.GnrlCnfgHdrID			= StrategyHeader.StrtgyID
		...
		Left Outer Join GeneralConfiguration GCProductGrade with (NoLock)
			on	GCProductGrade.GnrlCnfgTblNme		= 'Product'
			and	GCProductGrade.GnrlCnfgQlfr			= 'ProductGrade'
			and	GCProductGrade.GnrlCnfgHdrID		= Product.PrdctID
		Left Outer Join ReconcileGroupDetail RGDetail with (NoLock)
...
-- Removing per Mohammet - Defect 4169
--/*******************************************
--Get rid of any that didn't have any action
--********************************************/
--Delete	#InvBalance
--Where	RegradeProductName	is not null
--and		ABS(TotalReceipts) + ABS(TotalDisbursement) + ABS(GainLoss) + ABS(TotalBlend) = 0

--/*******************************************
--Get rid of zero-action base details where there's a regrade with action
--********************************************/
--Delete	#InvBalance
--Where	RegradeProductName	is null
--and		ABS(TotalReceipts) + ABS(TotalDisbursement) + ABS(GainLoss) + ABS(TotalBlend) = 0
--and		exists	(
--				Select	1
--				From	#InvBalance Sub
--				Where	Sub.DealNumber			= #InvBalance.DealNumber
--				And		Sub.DetailNumber		= #InvBalance.DetailNumber
--				And		Sub.RegradeProductName	is not null
--				)
