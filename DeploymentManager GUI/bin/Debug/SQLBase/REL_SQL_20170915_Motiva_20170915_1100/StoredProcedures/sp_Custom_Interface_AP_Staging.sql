If OBJECT_ID('dbo.Custom_Interface_AP_Staging') Is Not NULL
Begin
    DROP PROC dbo.Custom_Interface_AP_Staging
    PRINT '<<< DROPPED PROC dbo.Custom_Interface_AP_Staging >>>'
End
Go

Create Procedure dbo.Custom_Interface_AP_Staging	 @i_ID				int			= NULL
													,@c_OnlyShowSQL  	char(1) 	= 'N' 

As 
-----------------------------------------------------------------------------------------------------------------------------
-- Name:	Custom_Interface_AP_Staging @i_ID = 6, @c_OnlyShowSQL = 'Y'	Copyright 2003 SolArc
-- Overview:	
-- Arguments:		
-- SPs:
-- Temp Tables:
...
If OBJECT_ID('dbo.Custom_Interface_AP_Staging') Is NOT Null
BEGIN
	PRINT '<<< CREATED PROC dbo.Custom_Interface_AP_Staging >>>'
	Grant Execute on dbo.Custom_Interface_AP_Staging to SYSUSER
	Grant Execute on dbo.Custom_Interface_AP_Staging to RightAngleAccess
END
ELSE
	Print '<<<Failed Creating Procedure dbo.Custom_Interface_AP_Staging >>>'
GO