/*
*****************************************************************************************************
USE FIND AND REPLACE ON MTV_MTDIncrementalTaxTransactions WITH YOUR Function (NOTE:  Motiva_FN_ is already set
*****************************************************************************************************
*/

/****** Object:  StoredProcedure [dbo].[MTV_MTDIncrementalTaxTransactions]    Script Date: DATECREATED ******/
PRINT 'Start Script=sp_MTV_MTDIncrementalTaxTransactions.sql  Domain=Motiva  Time=' + CONVERT(VARCHAR(50), GETDATE(), 109) + ' on ' + @@SERVERNAME+'.'+db_name()
GO

IF  OBJECT_ID(N'[dbo].[MTV_MTDIncrementalTaxTransactions]') IS NULL
      BEGIN
			EXEC dbo.sp_executesql @statement = N'CREATE Procedure [dbo].[MTV_MTDIncrementalTaxTransactions](@In smalldatetime) AS SELECT 1'
			PRINT '<<< CREATED Procedure MTV_MTDIncrementalTaxTransactions >>>'
	  END
GO

/****** Object:  StoredProcedure [dbo].[MTV_MTDIncrementalTaxTransactions]    Script Date: 6/2/2016 8:20:27 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER OFF
GO

ALTER Procedure [dbo].[MTV_MTDIncrementalTaxTransactions]	@transactionsFromDate smalldatetime, 
															@userId int, 
															@accountingPeriodID int
As 

-----------------------------------------------------------------------------------------------------------------------------
-- Name:	MTV_MTDIncrementalTaxTransactions         Copyright 1997,1998,1999,2000,2001 SolArc
-- Overview:	Pull all the movement and the accounting transactions with the tax details.
-- Arguments:	
-- SPs:
-- Temp Tables:
-- Created by:	Sanjay Kumar
-- History:	12/15/2016 - First Created
--
...
--If	Object_ID('TempDB..#AccountDetailsIncludedToPEGA') Is Not Null
--BEGIN
--	Drop Table #AccountDetailsIncludedToPEGA
--	PRINT 'Dropped Temp Table #AccountDetailsIncludedToPEGA'
--END
...
TRUNCATE TABLE MTV_AccountDetailsIncludedToPEGA_MTDProcessing


--Log
INSERT	eventlog (Source,EventDateTime,EventType,Tier,SecurityIdentity,MachineName,Message,Data)
SELECT	'Month To Date Tax Transaction Interface'	,GETDATE(),'Start','SQL','Sysuser',db_name(),'Start','TRUNCATE MTVDataLakeMTDTaxTransactionStaging'

truncate table dbo.MTVDataLakeMTDTaxTransactionStaging

--Log
INSERT	eventlog (Source,EventDateTime,EventType,Tier,SecurityIdentity,MachineName,Message,Data)
SELECT	'Month To Date Tax Transaction Interface'	,GETDATE(),'Stop','SQL','Sysuser',db_name(),'Complete','TRUNCATE MTVDataLakeMTDTaxTransactionStaging'

--Log
INSERT	eventlog (Source,EventDateTime,EventType,Tier,SecurityIdentity,MachineName,Message,Data)
SELECT	'Month To Date Tax Transaction Interface'	,GETDATE(),'Start','SQL','Sysuser',db_name(),'Start','Gathering Tax Locale Structure'
...
--Log
INSERT	eventlog (Source,EventDateTime,EventType,Tier,SecurityIdentity,MachineName,Message,Data)
SELECT	'Month To Date Tax Transaction Interface'	,GETDATE(),'Stop','SQL','Sysuser',db_name(),'Complete','Gathering Tax Locale Structure'

--Log
INSERT	eventlog (Source,EventDateTime,EventType,Tier,SecurityIdentity,MachineName,Message,Data)
SELECT	'Month To Date Tax Transaction Interface'	,GETDATE(),'Start','SQL','Sysuser',db_name(),'Start','Insert TransactionHeaders into MTVDataLakeMTDTaxTransactionStaging'
...
Select	TaxLocale.LocaleState as MVT_TitleTransfer
	...
	From	[TransactionHeader] (NoLock)
		...
		Left Join TransactionDetail (NoLock) On
				TransactionDetail.XDtlXHdrID = (select top 1 XDtlXHdrID from TransactionDetail (Nolock) where XDtlXHdrID = TransactionHeader.XHdrID)
				And TransactionDetail.XDtlDlDtlPrvsnID = (select top 1 XDtlDlDtlPrvsnID from TransactionDetail (Nolock) where XDtlXHdrID = TransactionHeader.XHdrID)
				And TransactionDetail.XDtlID = (select top 1 XDtlID from TransactionDetail (Nolock) where XDtlXHdrID = TransactionHeader.XHdrID)
		...
		Left Join TransactionDetail As BilledUnits (NoLock)
					ON BilledUnits.XDtlXHdrID = (select top 1 XDtlXHdrID from TransactionDetail (Nolock) where XDtlXHdrID = TransactionHeader.XHdrID and XDtlLstInChn = 'Y')
					And BilledUnits.XDtlID = (select top 1 XDtlID from TransactionDetail (Nolock) where XDtlXHdrID = TransactionHeader.XHdrID and XDtlLstInChn = 'Y')
					And BilledUnits.XDtlDlDtlPrvsnID = (select top 1 XDtlDlDtlPrvsnID from TransactionDetail (Nolock) where XDtlXHdrID = TransactionHeader.XHdrID and XDtlLstInChn = 'Y')
...
--Log
INSERT	eventlog (Source,EventDateTime,EventType,Tier,SecurityIdentity,MachineName,Message,Data)
SELECT	'Month To Date Tax Transaction Interface'	,GETDATE(),'Start','SQL','Sysuser',db_name(),'Complete','Insert TransactionHeaders into MTVDataLakeMTDTaxTransactionStaging'

--Log
INSERT	eventlog (Source,EventDateTime,EventType,Tier,SecurityIdentity,MachineName,Message,Data)
SELECT	'Month To Date Tax Transaction Interface'	,GETDATE(),'Start','SQL','Sysuser',db_name(),'Start','Insert MovementHeader Transactions into MTVDataLakeMTDTaxTransactionStaging'
	...
	Select	TaxLocale.LocaleState as MVT_TitleTransfer
		...
		, (select stuff((
			...
			, null as Acct_GrossSignedVolume
			, null as Acct_BilledUOM
		--into ##MovementTransactionHeaderUpdates
	From MovementDocument (NoLock) 
		...
		Left Outer Join dbo.MTVDataLakeMTDTaxTransactionStaging (NoLock) On
			MTVDataLakeMTDTaxTransactionStaging.MVT_Transaction_ID = TransactionHeader.XHdrID
			and MTVDataLakeMTDTaxTransactionStaging.RecordType = 'M'
		Left Join [PlannedTransfer] (NoLock) On
			[TransactionHeader].[XHdrPlnndTrnsfrID] = [PlannedTransfer].[PlnndTrnsfrID]
		Left Join [DealHeader] (NoLock) On
			[PlannedTransfer].[PlnndTrnsfrObDlDtlDlHdrID] = [DealHeader].[DlHdrID]
	...
	and dbo.MTVDataLakeMTDTaxTransactionStaging.MVT_Transaction_ID is null

--Log
INSERT	eventlog (Source,EventDateTime,EventType,Tier,SecurityIdentity,MachineName,Message,Data)
SELECT	'Month To Date Tax Transaction Interface'	,GETDATE(),'Stop','SQL','Sysuser',db_name(),'Complete','Insert MovementHeader Transactions into MTVDataLakeMTDTaxTransactionStaging'
	--select * from ##MovementTransactionHeaderUpdates

/*
--Log
INSERT	eventlog (Source,EventDateTime,EventType,Tier,SecurityIdentity,MachineName,Message,Data)
SELECT	'Month To Date Tax Transaction Interface'	,GETDATE(),'Start','SQL','Sysuser',db_name(),'Start','Create AccountDetailsIncluded Temp Table'
	REPLACED WITH TABLE: MTV_AccountDetailsIncludedToPEGA_MTDProcessing
	-- Accounting transaction joins
	CREATE TABLE #AccountDetailsIncludedToPEGA(	[AcctDtlID] [int] NOT NULL,
	...
	CREATE NONCLUSTERED INDEX [AcctDtlSrceID_Idx] ON #AccountDetailsIncludedToPEGA (AcctDtlSrceID)
	CREATE NONCLUSTERED INDEX [AcctDtlAccntngPrdID_Idx] ON #AccountDetailsIncludedToPEGA (AcctDtlAccntngPrdID)
	CREATE NONCLUSTERED INDEX [AcctDtlSlsInvceHdrID_Idx] ON #AccountDetailsIncludedToPEGA (AcctDtlSlsInvceHdrID)
	CREATE NONCLUSTERED INDEX [AcctDtlPrchseInvceHdrID_Idx] ON #AccountDetailsIncludedToPEGA (AcctDtlPrchseInvceHdrID)
	CREATE NONCLUSTERED INDEX [InternalBAID_Idx] ON #AccountDetailsIncludedToPEGA (InternalBAID)
	CREATE NONCLUSTERED INDEX [AcctDtlDlDtlDlHdrID_Idx] ON #AccountDetailsIncludedToPEGA (AcctDtlDlDtlDlHdrID)
	CREATE NONCLUSTERED INDEX [AcctDtlMvtHdrID_Idx] ON #AccountDetailsIncludedToPEGA (AcctDtlMvtHdrID)
	CREATE NONCLUSTERED INDEX [AcctDtlLcleID_Idx] ON #AccountDetailsIncludedToPEGA (AcctDtlLcleID)
	CREATE NONCLUSTERED INDEX [AcctDtlDestinationLcleID_Idx] ON #AccountDetailsIncludedToPEGA (AcctDtlDestinationLcleID)
	CREATE NONCLUSTERED INDEX [AcctDtlSrceTble_Idx] ON #AccountDetailsIncludedToPEGA (AcctDtlSrceTble)
	CREATE NONCLUSTERED INDEX [AcctDtlTrnsctnTypID_Idx] ON #AccountDetailsIncludedToPEGA (AcctDtlTrnsctnTypID)
	CREATE NONCLUSTERED INDEX [AcctDtlClseStts_Idx] ON #AccountDetailsIncludedToPEGA (AcctDtlClseStts)
	CREATE NONCLUSTERED INDEX [AcctDtlAcctCdeStts_Idx] ON #AccountDetailsIncludedToPEGA (AcctDtlAcctCdeStts)
	CREATE NONCLUSTERED INDEX [AcctDtlTxStts_Idx] ON #AccountDetailsIncludedToPEGA (AcctDtlTxStts)
	CREATE NONCLUSTERED INDEX [AcctDtlPrntID_Idx] ON #AccountDetailsIncludedToPEGA (AcctDtlPrntID)
	CREATE NONCLUSTERED INDEX [AcctDtlTrnsctnDte_Idx] ON #AccountDetailsIncludedToPEGA (AcctDtlTrnsctnDte)
	CREATE NONCLUSTERED INDEX [ExternalBAID_Idx] ON #AccountDetailsIncludedToPEGA (ExternalBAID)
	CREATE NONCLUSTERED INDEX [ParentPrdctID_Idx] ON #AccountDetailsIncludedToPEGA (ParentPrdctID)
	CREATE NONCLUSTERED INDEX [PayableMatchingStatus_Idx] ON #AccountDetailsIncludedToPEGA (PayableMatchingStatus)
	CREATE NONCLUSTERED INDEX [CreatedDate_Idx] ON #AccountDetailsIncludedToPEGA (CreatedDate)

--Log
INSERT	eventlog (Source,EventDateTime,EventType,Tier,SecurityIdentity,MachineName,Message,Data)
SELECT	'Month To Date Tax Transaction Interface'	,GETDATE(),'Stop','SQL','Sysuser',db_name(),'Complete','Create AccountDetailsIncluded Temp Table'
*/

--Log
INSERT	eventlog (Source,EventDateTime,EventType,Tier,SecurityIdentity,MachineName,Message,Data)
SELECT	'Month To Date Tax Transaction Interface'	,GETDATE(),'Start','SQL','Sysuser',db_name(),'Start','Create TaxRates Temp Table'
...
--Log
INSERT	eventlog (Source,EventDateTime,EventType,Tier,SecurityIdentity,MachineName,Message,Data)
SELECT	'Month To Date Tax Transaction Interface'	,GETDATE(),'Stop','SQL','Sysuser',db_name(),'Complete','Create TaxRates Temp Table'

--Log
INSERT	eventlog (Source,EventDateTime,EventType,Tier,SecurityIdentity,MachineName,Message,Data)
SELECT	'Month To Date Tax Transaction Interface'	,GETDATE(),'Start','SQL','Sysuser',db_name(),'Start','Filter Accounting Transactions not in Transaction Group ExcludedFromPega'

	-- Filter the Accounting transactions for all types that do not belong to the Transaction Group ExcludedFromPEGA
	Insert MTV_AccountDetailsIncludedToPEGA_MTDProcessing(AcctDtlID,
										 AcctDtlSrceID,
										 AcctDtlTrnsctnTypID,
										 AcctDtlAccntngPrdID,
	...
	select	AcctDtlID,
			...
			Reversed,
			SupplyDemand
			from AccountDetail  (Nolock) where AcctDtlTrnsctnTypID not in
...
--Log
INSERT	eventlog (Source,EventDateTime,EventType,Tier,SecurityIdentity,MachineName,Message,Data)
SELECT	'Month To Date Tax Transaction Interface'	,GETDATE(),'Stop','SQL','Sysuser',db_name(),'Complete','Filter Accounting Transactions not in Transaction Group ExcludedFromPega'

--Log
INSERT	eventlog (Source,EventDateTime,EventType,Tier,SecurityIdentity,MachineName,Message,Data)
SELECT	'Month To Date Tax Transaction Interface'	,GETDATE(),'Start','SQL','Sysuser',db_name(),'Start','Gather Accounting Transactions'

	Select	TaxLocale.LocaleState as MVT_TitleTransfer
		,abs(cast(MTV_AccountDetailsIncludedToPEGA_MTDProcessing.Volume as decimal(15,2))) As MVT_BilledUnits
		,MovementDocument.MvtDcmntExtrnlDcmntNbr As MVT_BillOfLading
		,MovementDocument.MvtDcmntIntrnlDcmntNbr As MVT_InternalDocumentNumber
		,TransactionHeader.MovementDate As MVT_BillOfLadingDate
		,case when (MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble = 'X') then TransactionHeader.XHdrGrssQty else TaxTransactionHeader.XHdrGrssQty end as MVT_GrossUnits
		...
		,case when (MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble = 'X') then TransactionHeader.XHdrQty else TaxTransactionHeader.XHdrQty end as MVT_NetUnits
		,ProductCode.GnrlCnfgMulti As MVT_ProductCode
		,case when (MTV_AccountDetailsIncludedToPEGA_MTDProcessing.Volume != 0) then ABS(convert(DECIMAL(15,6), MTV_AccountDetailsIncludedToPEGA_MTDProcessing.Value/MTV_AccountDetailsIncludedToPEGA_MTDProcessing.Volume)) else 0 end as MVT_UnitPrice
		...
		,Vehicle.VhcleNme As MVT_Vessel_Name
		,case when(Vehicle.VhcleTpe = 'V') then ISNULL(VehicleVessel.USCGID, VehicleVessel.ExternalSourceID) else ISNULL(VehicleBarge.USCGID,VehicleBarge.ExternalSourceID) end As MVT_Vessel_Number
		,case when (MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble = 'X') then cast(TransactionHeader.XHdrDte as date) else cast(TaxTransactionHeader.XHdrDte as date) end as MVT_Movement_Posted_Date

		,case when (MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble = 'X') then DATEPART(year, TransactionHeader.XHdrDte) else DATEPART(year, TaxTransactionHeader.XHdrDte) end as MVT_Transaction_Year
		,case when (MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble = 'X') then DATEPART(month, TransactionHeader.XHdrDte) else DATEPART(month, TaxTransactionHeader.XHdrDte) end as MVT_Transaction_Month
		,case when (MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble = 'X') then convert(int, TransactionHeader.XHdrID) else convert(int, TaxTransactionHeader.XHdrID) end As MVT_Transaction_ID
		,case when (MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble = 'X') then convert(int, TransactionHeader.XHdrChldXHdrID) else convert(int, TaxTransactionHeader.XHdrChldXHdrID) end as MVT_Transaction_Child_Id

		,DATEPART(year, MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlTrnsctnDte) As MVT_Accounting_Year
		,DATEPART(month, MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlTrnsctnDte) As MVT_Accounting_Month
		
		,TransactionDetail.XDtlID As MVT_BOL_Item_Number
		,case when (MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble = 'X') then TransactionHeader.XHdrTyp else TaxTransactionHeader.XHdrTyp end as MVT_RD_Type
		,DealType.Description As MVT_Deal_Type
		,CommoditySubGroup.Name As MVT_Tax_Commodity_Code
		,MovementHeaderType.Name As MVT_Movement_Type
		,DealHeader.DlHdrIntrnlNbr As MVT_Deal
		...
		,MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlID As MVT_Account_DetailID
		,ExternalBA.BANme As MVT_ExternalBA
		,SAPSoldTo.SoldTo As MVT_GSAP_CustomerNumber
		,SAPSoldTo.VendorNumber As MVT_GSAP_VendorNumber
		,ExtBASCAC.GnrlCnfgMulti As MVT_SCAC
		,ProductMaterialCode.GnrlCnfgMulti As MVT_SAP_Material_Code
		,case when (VehicleVessel.USCGID is not null OR VehicleBarge.USCGID is not null) then 'No' else 'Yes' end as MVT_Foreign_Vessel_Identifier
		,case when (MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble = 'X' AND TransactionHeader.XHdrStat = 'C') then 'Complete' when (MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble = 'X' AND TransactionHeader.XHdrStat = 'R') then 'Reversed' 
		 when (MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble != 'X' AND TaxTransactionHeader.XHdrStat = 'C') then 'Complete' when (MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble != 'X' AND TaxTransactionHeader.XHdrStat = 'R') then 'Reversed' end as MVT_Transaction_Status
		,ProductGrade.GnrlCnfgMulti As MVT_Product_Grade
		,convert(int, MovementHeader.MvtHdrLcleID) as MVT_Location_ID
		,Product.PrdctNme As MVT_Product_Name
		...
		------Accounting Fields
		,case when (MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble = 'X') then convert(int, TransactionHeader.XHdrID) else convert(int, TaxTransactionHeader.XHdrID) end as Acct_MVT_Transaction_Unique_Identifier
		,case when (MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble = 'X') then convert(int, TransactionHeader.XHdrChldXHdrID) else convert(int, TaxTransactionHeader.XHdrChldXHdrID) end as Acct_MVT_Transaction_Child_Id
		,convert(int, MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlID) As Acct_DtlID
		,ExternalBA.BANme As Acct_ExtBA
		,DealHeader.DlHdrIntrnlNbr As Acct_Deal
		,MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlTrnsctnDte As Acct_TktDate
		,MovementDocument.MvtDcmntExtrnlDcmntNbr As Acct_BOL
		,case when (MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble = 'X') then TransactionHeader.MovementDate else TaxTransactionHeader.MovementDate end As Acct_BOL_Date
		...
		,case when (MTV_AccountDetailsIncludedToPEGA_MTDProcessing.Volume = MTV_AccountDetailsIncludedToPEGA_MTDProcessing.NetQuantity) then 'N' else 'G' end as Acct_NG
		,convert(DECIMAL(20, 10), #TaxRates.TaxRate) as Acct_Rate
		,convert(DECIMAL(15,3), MTV_AccountDetailsIncludedToPEGA_MTDProcessing.Value) As Acct_Tran_Amt

		,OriginLocale.LcleAbbrvtn As Acct_Origin
		,DestinationLocale.LcleAbbrvtn As Acct_Destination
		,SalesInvoiceHeader.SlsInvceHdrNmbr as Acct_SlsInv
		,MTV_AccountDetailsIncludedToPEGA_MTDProcessing.NetQuantity as Acct_Net
		,MTV_AccountDetailsIncludedToPEGA_MTDProcessing.GrossQuantity as Acct_Gross
		...
		,MTV_AccountDetailsIncludedToPEGA_MTDProcessing.Reversed as Acct_Rev
		,case when (MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlTxStts = 'R') then 'Evaluated' else 'Not Evaluated' end as Acct_Taxed
		,MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble as Acct_Source
		...
		,MTV_AccountDetailsIncludedToPEGA_MTDProcessing.PayableMatchingStatus as Acct_Payable_Matching_Status
		,MovementHeaderType.Name as Acct_MovementType
		,DealType.Description as Acct_DealType
		,OriginTaxLocale.LocaleState as Acct_OriginState
		,DestinationTaxLocale.LocaleState as Acct_DestinationState
		,MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlAcctCdeStts as Acct_AcctCodeStatus
		...
		,MTV_AccountDetailsIncludedToPEGA_MTDProcessing.Volume as Acct_BilledGallons
		,case when (MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble = 'X') then TransactionHeader.XHdrTyp else TaxTransactionHeader.XHdrTyp end as Acct_R_D
		,MTV_AccountDetailsIncludedToPEGA_MTDProcessing.Reversed as Acct_Reversed
		,convert(varchar(50), DATEPART(month, TransactionHeader.MovementDate)) + "/" + convert(varchar(50),DATEPART(year, TransactionHeader.MovementDate)) as Acct_Movement_Year_Month
		,MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlTrnsctnDte as Acct_Movement_Posting_Date
		,MTV_AccountDetailsIncludedToPEGA_MTDProcessing.CreatedDate as Acct_GL_Posting_Date
		,convert(int, Buyer.BAID) as Acct_Buyer_CustomID
		,convert(int, Seller.BAID) as Acct_Seller_CustomID
		,ExternalBA.Name as Acct_External_BA
		,convert(int, ExternalBA.BAID) as Acct_External_BAID
		...
		,MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlTxStts as Acct_Detail_Status
		,MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlPrntID as Account_Detail_ParentID
		,Users.UserDBMnkr as MVT_Changed_By
		,TaxLocale.LocaleCounty as MVT_TitleTransferCounty
		,TaxLocale.LocaleCity as MVT_TitleTransferCity
		, (select stuff((
	...
	From	MTV_AccountDetailsIncludedToPEGA_MTDProcessing (NoLock)
	Inner JOIN AccountDetail (NOLOCK) ON AccountDetail.AcctDtlID = MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlID
		...
		Left Join [TransactionDetailLog] (NoLock) On
			MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceID = [TransactionDetailLog].XDtlLgID And MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble = 'X'
		Left Join [TransactionHeader] (NoLock) On
			TransactionDetailLog.XDtlLgXDtlXHdrID = TransactionHeader.XHdrID
		Left Join TaxDetailLog (NoLock) On
			MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceID = [TaxDetailLog].TxDtlLgID And MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble = 'T'
		...
		Left Join [PlannedTransfer] (NoLock) On
			[PlannedTransfer].[PlnndTrnsfrID] = case when (MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble = 'X') then TransactionHeader.XHdrPlnndTrnsfrID
													 else TaxTransactionHeader.XHdrPlnndTrnsfrID end
		Left Join DealHeader (NoLock) On
			[PlannedTransfer].[PlnndTrnsfrObDlDtlDlHdrID] = [DealHeader].[DlHdrID]
		Left Join [MovementHeader] (NoLock) On
			MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlMvtHdrID = [MovementHeader].[MvtHdrID]
		Left join [DealDetail] (NoLock) On
				DealDetail.DealDetailID = case when (MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble = 'X') then TransactionHeader.DealDetailID
													else TaxTransactionHeader.DealDetailID end
		Left Join [BusinessAssociate] As Buyer (NoLock) On
				Buyer.BAID = Case	When MTV_AccountDetailsIncludedToPEGA_MTDProcessing.SupplyDemand = 'R' Then  MTV_AccountDetailsIncludedToPEGA_MTDProcessing.InternalBAID
									When MTV_AccountDetailsIncludedToPEGA_MTDProcessing.SupplyDemand = 'D' Then  MTV_AccountDetailsIncludedToPEGA_MTDProcessing.ExternalBAID 
							Else	MovementHeader.MvtHdrSrchBAID 	End
		Left Join GeneralConfiguration As BuyerFEIN (NoLock) On
				BuyerFEIN.GnrlCnfgQlfr = 'FederalTaxID'
				And BuyerFEIN.GnrlCnfgTblNme = 'BusinessAssociate'
		...
		Left Join [BusinessAssociate] As Seller (NoLock) On
				Seller.BAID = Case	When MTV_AccountDetailsIncludedToPEGA_MTDProcessing.SupplyDemand = 'D' Then  MTV_AccountDetailsIncludedToPEGA_MTDProcessing.InternalBAID
									When MTV_AccountDetailsIncludedToPEGA_MTDProcessing.SupplyDemand = 'R' Then  MTV_AccountDetailsIncludedToPEGA_MTDProcessing.ExternalBAID 
							  Else MovementHeader.MvtHdrSrchBAID End
		Left Join [GeneralConfiguration] As SellerFEIN (NoLock) On
				SellerFEIN.[GnrlCnfgQlfr] = 'FederalTaxID'
				And SellerFEIN.[GnrlCnfgTblNme] = 'BusinessAssociate'
		...
		Left Join Locale As OriginLocale (NoLock) On
				MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlLcleID = OriginLocale.LcleID
		Left Join LocaleType As OriginLocaleType (NoLock) On
				OriginLocale.LcleTpeID = OriginLocaleType.LcleTpeID
		Left Join #TaxLocaleStructure As OriginTaxLocale (NoLock) On
				OriginTaxLocale.LcleID = MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlLcleID
		Left Join GeneralConfiguration As OriginTerminalCode (NoLock) On
				OriginTerminalCode.GnrlCnfgQlfr = 'TCN'
				And OriginTerminalCode.GnrlCnfgTblNme = 'Locale'
				And OriginTerminalCode.GnrlCnfgHdrID = MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlLcleID
		Left Join [GeneralConfiguration] As OriginCountyCode (NoLock) On
				OriginCountyCode.[GnrlCnfgQlfr] = 'USFedCountyCode'
				And OriginCountyCode.[GnrlCnfgTblNme] = 'Locale'
				And OriginCountyCode.[GnrlCnfgHdrID] = MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlLcleID
		Left Join [GeneralConfiguration] As OriginCountryCode (NoLock) On
				OriginCountryCode.[GnrlCnfgQlfr] = 'CountryCode'
				And OriginCountryCode.[GnrlCnfgTblNme] = 'Locale'
				And OriginCountryCode.[GnrlCnfgHdrID] =  MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlLcleID
		...
		Left Join MTVSAPSoldToShipTo As DestinationSoldToShipTo (NoLock) On
				DestinationSoldToShipTo.MTVSAPBASoldToID = SAPBASoldTo.ID
				And DestinationSoldToShipTo.RALocaleID = MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlDestinationLcleID

		Left Join [GeneralConfiguration] As OriginZipCode (NoLock) On
				OriginZipCode.[GnrlCnfgQlfr] = 'PostalCode'
				And OriginZipCode.[GnrlCnfgTblNme] = 'Locale'
				And OriginZipCode.[GnrlCnfgHdrID] =  MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlLcleID

		Left Join Locale As DestinationLocale (NoLock) On
				MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlDestinationLcleID = DestinationLocale.LcleID
		Left Join LocaleType As DestinationLocaleType (NoLock) On
				DestinationLocale.LcleTpeID = DestinationLocaleType.LcleTpeID
		Left Join #TaxLocaleStructure As DestinationTaxLocale (NoLock) On
				DestinationTaxLocale.LcleID = MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlDestinationLcleID
		Left Join [GeneralConfiguration] As DestinationTerminalCode (NoLock) On
				DestinationTerminalCode.[GnrlCnfgQlfr] = 'TCN'
				And DestinationTerminalCode.[GnrlCnfgTblNme] = 'Locale'
				And DestinationTerminalCode.[GnrlCnfgHdrID] = MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlDestinationLcleID

		Left Join GeneralConfiguration As DestinationAddress1 (NoLock) On
				DestinationAddress1.[GnrlCnfgQlfr] = 'AddrLine1'
				And DestinationAddress1.[GnrlCnfgTblNme] = 'Locale'
				And DestinationAddress1.[GnrlCnfgHdrID] = MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlDestinationLcleID
		Left Join [GeneralConfiguration] As DestinationAddress2 (NoLock) On
				DestinationAddress2.[GnrlCnfgQlfr] = 'AddrLine2'
				And DestinationAddress2.[GnrlCnfgTblNme] = 'Locale'
				And DestinationAddress2.[GnrlCnfgHdrID] = MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlDestinationLcleID
		Left Join [GeneralConfiguration] As DestinationCountyCode (NoLock) On
				DestinationCountyCode.[GnrlCnfgQlfr] = 'USFedCountyCode'
				And DestinationCountyCode.[GnrlCnfgTblNme] = 'Locale'
				And DestinationCountyCode.[GnrlCnfgHdrID] = MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlDestinationLcleID
		Left Join [GeneralConfiguration] As DestinationCountryCode (NoLock) On
				DestinationCountryCode.[GnrlCnfgQlfr] = 'CountryCode'
				And DestinationCountryCode.[GnrlCnfgTblNme] = 'Locale'
				And DestinationCountryCode.[GnrlCnfgHdrID] = MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlDestinationLcleID
		Left Join [GeneralConfiguration] As DestinationZipCode (NoLock) On
				DestinationZipCode.[GnrlCnfgQlfr] = 'PostalCode'
				And DestinationZipCode.[GnrlCnfgTblNme] = 'Locale'
				And DestinationZipCode.[GnrlCnfgHdrID] = MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlDestinationLcleID
...
-- Start the joins with the Accounting tables this will result in pulling all the Accounting txns for each Transaction Header ... Transaction Detail (all provisions) 1... 1. Account Detail
		Left Join TransactionDetail (NoLock) On
				TransactionDetail.TransactionDetailID = case 
															when (MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble = 'X') then TransactionDetailLog.XDtlLgXDtlID
															else TaxDetailLog.TxDtlLgTxDtlID end
		Left Join SalesInvoiceHeader (NoLock) On	
				SalesInvoiceHeader.SlsInvceHdrID = MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSlsInvceHdrID
		Left Join PayableHeader (NoLock) On
				PayableHeader.PybleHdrID = MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlPrchseInvceHdrID
		Left Join TransactionType (NoLock) On
				MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlTrnsctnTypID = TransactionType.TrnsctnTypID
		Left Join Locale As AccountDetailLocation (NoLock) On
				AccountDetailLocation.LcleID = MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlLcleID
		Left Join AccountingPeriod (NoLock) On
				MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlAccntngPrdID = AccountingPeriod.AccntngPrdID
		Left Join Currency (NoLock) On
				Currency.CrrncyID = MTV_AccountDetailsIncludedToPEGA_MTDProcessing.CrrncyID
		Left Join BusinessAssociate As ExternalBA (NoLock) On
				MTV_AccountDetailsIncludedToPEGA_MTDProcessing.ExternalBAID = ExternalBA.BAID
		Left Join GeneralConfiguration As ExtBASAPCustomerNumber (NoLock) On
				ExtBASAPCustomerNumber.GnrlCnfgQlfr = 'SAPSoldTo'
				And ExtBASAPCustomerNumber.GnrlCnfgTblNme = 'DealHeader'
				And ExtBASAPCustomerNumber.GnrlCnfgHdrID = MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlDlDtlDlHdrID
				And ExtBASAPCustomerNumber.GnrlCnfgMulti != 'X'
		Left Join MTVSAPBASoldTo As SAPSoldTo (NoLock) On
				SAPSoldTo.ID = ExtBASAPCustomerNumber.GnrlCnfgMulti
		Left Join [GeneralConfiguration] As ExtBASCAC (NoLock) On
		...
		Left Join BusinessAssociate As InternalBA (NoLock) On
				MTV_AccountDetailsIncludedToPEGA_MTDProcessing.InternalBAID = InternalBA.BAID
		Left Join #TaxLocaleStructure As TaxLocale (NoLock) On
				TaxLocale.LcleID = MovementHeader.MvtHdrLcleID
		Left Join Locale As TaxRALocale (NoLock) On
		...
		Left Join #TaxRates (NoLock) On
				MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlID = #TaxRates.AcctDtlID
		Left Join Users (NoLock) On
				MovementHeader.UserID = Users.UserID

		Left Join MovementHeader As DocMovementHeader (NoLock) On
				DocMovementHeader.MvtHdrID = case when (MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble = 'X') then TransactionHeader.XHdrMvtDtlMvtHdrID 
		...
		Left Join GeneralConfiguration As ShipTo (NoLock) On
				...
				And ShipTo.GnrlCnfgHdrID = (select AcctDtlMvtHdrID from AccountDetail where AcctDtlId = MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlID)
...
--Log
INSERT	eventlog (Source,EventDateTime,EventType,Tier,SecurityIdentity,MachineName,Message,Data)
SELECT	'Month To Date Tax Transaction Interface'	,GETDATE(),'Stop','SQL','Sysuser',db_name(),'Complete','Gather Accounting Transactions'
						
--Log
INSERT	eventlog (Source,EventDateTime,EventType,Tier,SecurityIdentity,MachineName,Message,Data)
SELECT	'Month To Date Tax Transaction Interface'	,GETDATE(),'Start','SQL','Sysuser',db_name(),'Start','Insert Accounting Transactions'
	...
	From MTVDataLakeMTDTaxTransactionStaging AT
	inner join #MTV_AccountDetailsWithStrategy ADS on AT.AcctDtlId = ADS.AcctDtlId

	--select count(*) As ATForOpenPeriod from MTVDataLakeMTDTaxTransactionStaging

--Log
INSERT	eventlog (Source,EventDateTime,EventType,Tier,SecurityIdentity,MachineName,Message,Data)
SELECT	'Month To Date Tax Transaction Interface'	,GETDATE(),'Stop','SQL','Sysuser',db_name(),'Complete','Insert Accounting Transactions'

	select @closeDate=CloseDate from AccountingPeriod  (NoLock) where AccntngPrdID=@accountingPeriodID-1

--Log
INSERT	eventlog (Source,EventDateTime,EventType,Tier,SecurityIdentity,MachineName,Message,Data)
SELECT	'Month To Date Tax Transaction Interface'	,GETDATE(),'Start','SQL','Sysuser',db_name(),'Start','Gather Sales Invoice Generated from Previous Account Periods'
...
AND		NOT EXISTS (
					SELECT	'X' 
					FROM	MTVDataLakeMTDTaxTransactionStaging (NOLOCK)
					WHERE	accountdetail.AcctDtlID = MTVDataLakeMTDTaxTransactionStaging.AcctDtlID
					)
ORDER BY EntityID
...
--Log
INSERT	eventlog (Source,EventDateTime,EventType,Tier,SecurityIdentity,MachineName,Message,Data)
SELECT	'Month To Date Tax Transaction Interface'	,GETDATE(),'Stop','SQL','Sysuser',db_name(),'Complete','Gather Sales Invoices Generated from Previous Account Periods'

INSERT	eventlog (Source,EventDateTime,EventType,Tier,SecurityIdentity,MachineName,Message,Data)
SELECT	'Month To Date Tax Transaction Interface'	,GETDATE(),'Start','SQL','Sysuser',db_name(),'Start','Insert Sales Invoice Generated from Previous Account Periods'
...
--AND		AccountDetail.AcctDtlSlsInvceHdrID is not null
	...
	INSERT INTO MTVDataLakeMTDTaxTransactionStaging
...
--Log
INSERT	eventlog (Source,EventDateTime,EventType,Tier,SecurityIdentity,MachineName,Message,Data)
SELECT	'Month To Date Tax Transaction Interface'	,GETDATE(),'Stop','SQL','Sysuser',db_name(),'Complete','Insert Sales Invoice Generated from Previous Account Periods'

--Log
INSERT	eventlog (Source,EventDateTime,EventType,Tier,SecurityIdentity,MachineName,Message,Data)
SELECT	'Month To Date Tax Transaction Interface'	,GETDATE(),'Start','SQL','Sysuser',db_name(),'Start','Gather Purchase Invoice Generated from Previous Account Periods'
...
INSERT	eventlog (Source,EventDateTime,EventType,Tier,SecurityIdentity,MachineName,Message,Data)
SELECT	'Month To Date Tax Transaction Interface'	,GETDATE(),'Stop','SQL','Sysuser',db_name(),'Complete','Gather Purchase Invoice Generated from Previous Account Periods'

--Log
INSERT	eventlog (Source,EventDateTime,EventType,Tier,SecurityIdentity,MachineName,Message,Data)
SELECT	'Month To Date Tax Transaction Interface'	,GETDATE(),'Start','SQL','Sysuser',db_name(),'Start','Insert Purchase Invoice Generated from Previous Account Periods'
--INSERT INTO dbo.MTVDataLakeMTDTaxTransactionStaging
SELECT 	TaxLocale.LocaleState as MVT_TitleTransfer,
		ABS(cast(AccountDetail.Volume as decimal(15,2))) As MVT_BilledUnits
		,MovementDocument.MvtDcmntExtrnlDcmntNbr As MVT_BillOfLading
...
--Log
INSERT	eventlog (Source,EventDateTime,EventType,Tier,SecurityIdentity,MachineName,Message,Data)
SELECT	'Month To Date Tax Transaction Interface'	,GETDATE(),'Stop','SQL','Sysuser',db_name(),'Complete','Insert Purchase Invoice Generated from Previous Account Periods'

--select count(*) as PurchaseInvGen from MTVDataLakeMTDTaxTransactionStaging


--Log
INSERT	eventlog (Source,EventDateTime,EventType,Tier,SecurityIdentity,MachineName,Message,Data)
SELECT	'Month To Date Tax Transaction Interface'	,GETDATE(),'Start','SQL','Sysuser',db_name(),'Start','Insert MTV_InvoiceDetail_AccountDetail_DeleteLog information'
	...
	Select	TaxLocale.LocaleState as MVT_TitleTransfer
		...
		,case when (MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble = 'X') then DATEPART(year, TransactionHeader.XHdrDte) else DATEPART(year, TaxTransactionHeader.XHdrDte) end as MVT_Transaction_Year
		,case when (MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble = 'X') then DATEPART(month, TransactionHeader.XHdrDte) else DATEPART(month, TaxTransactionHeader.XHdrDte) end as MVT_Transaction_Month
		,case when (MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble = 'X') then TransactionHeader.XHdrID else TaxTransactionHeader.XHdrID end As MVT_Transaction_ID
		,case when (MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble = 'X') then TransactionHeader.XHdrChldXHdrID else TaxTransactionHeader.XHdrChldXHdrID end as MVT_Transaction_Child_Id
		...
		------Accounting Fields
		,case when (MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble = 'X') then TransactionHeader.XHdrID else TaxTransactionHeader.XHdrID end as Acct_MVT_Transaction_Unique_Identifier
		,case when (MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble = 'X') then TransactionHeader.XHdrChldXHdrID else TaxTransactionHeader.XHdrChldXHdrID end as Acct_MVT_Transaction_Child_Id
		,MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlID As Acct_DtlID
		,ExternalBA.BANme As Acct_ExtBA
		,DealHeader.DlHdrIntrnlNbr As Acct_Deal
		,MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlTrnsctnDte As Acct_TktDate
		,MovementDocument.MvtDcmntExtrnlDcmntNbr As Acct_BOL
	...
	From	MTV_InvoiceDetail_AccountDetail_DeleteLog DLog (NoLock)
		...
		LEFT JOIN dbo.Unitofmeasure (NOLOCK) ON Unitofmeasure.UOM = AccountDetail.AcctDtlUOMID
		Left Join MTV_AccountDetailsIncludedToPEGA_MTDProcessing (NoLock) On
			DLog.AccountDetailID = MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlId
		Left Outer Join #AccountingTransactions (NoLock) On
			#AccountingTransactions.Acct_DtlID = MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlID
		Left Join [TransactionDetailLog] (NoLock) On
			MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceID = TransactionDetailLog.XDtlLgID And MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble = 'X'
		Left Join [TransactionHeader] (NoLock) On
			TransactionDetailLog.XDtlLgXDtlXHdrID = TransactionHeader.XHdrID
		...
		Left Join [PlannedTransfer] (NoLock) On
			[PlannedTransfer].[PlnndTrnsfrID] = case 
													when (MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlSrceTble = 'X') then TransactionHeader.XHdrPlnndTrnsfrID
													else TaxTransactionHeader.XHdrPlnndTrnsfrID end
		Left Join DealHeader (NoLock) On
			[PlannedTransfer].[PlnndTrnsfrObDlDtlDlHdrID] = [DealHeader].[DlHdrID]
		Left Join [MovementHeader] (NoLock) On
...
-- Start the joins with the Accounting tables this will result in pulling all the Accounting txns for each Transaction Header ... Transaction Detail (all provisions) 1... 1. Account Detail
	...
	and MTV_AccountDetailsIncludedToPEGA_MTDProcessing.AcctDtlId is not null
...
--Log
INSERT	eventlog (Source,EventDateTime,EventType,Tier,SecurityIdentity,MachineName,Message,Data)
SELECT	'Month To Date Tax Transaction Interface'	,GETDATE(),'Stop','SQL','Sysuser',db_name(),'Complete','Insert MTV_InvoiceDetail_AccountDetail_DeleteLog information'
...
IF  OBJECT_ID(N'[dbo].[MTV_MTDIncrementalTaxTransactions]') IS NOT NULL
      BEGIN
			EXECUTE	sp_MotivaBuildStatisticsInsertUpdateSQLScripts 'sp_MTV_MTDIncrementalTaxTransactions.sql'
			PRINT '<<< ALTERED Procedure MTV_MTDIncrementalTaxTransactions >>>'
	  END
	  ELSE
	  BEGIN
			PRINT '<<< FAILED CREATE OR ALTER on Procedure MTV_MTDIncrementalTaxTransactions >>>'
	  END
...
--End of Script: sp_MTV_MTDIncrementalTaxTransactions.sql
