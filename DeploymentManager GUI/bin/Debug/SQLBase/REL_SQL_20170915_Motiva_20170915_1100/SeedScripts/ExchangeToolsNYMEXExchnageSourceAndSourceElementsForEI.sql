...
---------------------------------------------------------------------------------------
-- To configure entries for ExchangeToolsNYMEX exchange source with all the source elements
---------------------------------------------------------------------------------------


DECLARE		@Key					INT
DECLARE		@SrcSystemId			INT
DECLARE		@ExchangeToolsNYMEX		VARCHAR(30)		= 'ExchangeToolsNYMEX'
...
---------------------------------------------------------------------------------------
-- Insert SourceSystem record for ExchangeToolsNYMEX
---------------------------------------------------------------------------------------
IF EXISTS(SELECT 1 FROM SourceSystem WHERE Name = @ExchangeToolsNYMEX)
BEGIN
	SELECT @SrcSystemId = SrceSystmID FROM SourceSystem WHERE Name = @ExchangeToolsNYMEX
END
ELSE
BEGIN
	INSERT INTO SourceSystem (Name, SrceSystmObjct)
	VALUES(@ExchangeToolsNYMEX, NULL);
