...
---------------------------------------------------------------------------------------
-- To configure entries for DME exchange source with all the source elements
---------------------------------------------------------------------------------------


DECLARE		@Key					INT
DECLARE		@SrcSystemId			INT
DECLARE		@DME					VARCHAR(30)		= 'DME'
...
---------------------------------------------------------------------------------------
-- Insert SourceSystem record for DME
---------------------------------------------------------------------------------------
IF EXISTS(SELECT 1 FROM SourceSystem WHERE Name = @DME)
BEGIN
	SELECT @SrcSystemId = SrceSystmID FROM SourceSystem WHERE Name = @DME
END
ELSE
BEGIN
