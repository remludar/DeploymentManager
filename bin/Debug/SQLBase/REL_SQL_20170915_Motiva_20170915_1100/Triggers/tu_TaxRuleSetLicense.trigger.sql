...
IF OBJECT_ID('dbo.tu_TaxRuleSetLicense_MPC') IS NOT NULL
  BEGIN
    DROP TRIGGER dbo.tu_TaxRuleSetLicense_MPC
    IF OBJECT_ID('dbo.tu_TaxRuleSetLicense_MPC') IS NOT NULL
        PRINT '<<< FAILED DROPPING TRIGGER dbo.tu_TaxRuleSetLicense_MPC >>>'
    ELSE
        PRINT '<<< DROPPED TRIGGER dbo.tu_TaxRuleSetLicense_MPC >>>'
  END
go


CREATE trigger tu_TaxRuleSetLicense_MPC on dbo.TaxRuleSetLicense AFTER UPDATE as
begin

-----------------------------------------------------------------------------------------------------------------------------
-- Name:       tu_TaxRuleSetLicense_MPC
-- Arguments: 
-- Tables:    
-- Indexes:   
-- SPs:        
-- Overview:   This custom update trigger will write records to the custom MPC_TaxAudit table to maintain
...
--
-- Insert a record into the MPC_TaxAudit table for each updated field on each changed record
--
...
IF OBJECT_ID('dbo.tu_TaxRuleSetLicense_MPC') IS NOT NULL
  PRINT '<<< CREATED TRIGGER dbo.tu_TaxRuleSetLicense_MPC >>>'
ELSE
  PRINT '<<< FAILED CREATING TRIGGER dbo.tu_TaxRuleSetLicense_MPC >>>'
