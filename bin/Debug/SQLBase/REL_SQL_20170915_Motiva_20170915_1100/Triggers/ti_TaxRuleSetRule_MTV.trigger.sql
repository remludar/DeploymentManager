...
IF OBJECT_ID('dbo.ti_TaxRuleSetRule_MTV') IS NOT NULL
  BEGIN
    DROP TRIGGER dbo.ti_TaxRuleSetRule_MTV
    IF OBJECT_ID('dbo.ti_TaxRuleSetRule_MTV') IS NOT NULL
        PRINT '<<< FAILED DROPPING TRIGGER dbo.ti_TaxRuleSetRule_MTV >>>'
    ELSE
        PRINT '<<< DROPPED TRIGGER dbo.ti_TaxRuleSetRule_MTV >>>'
  END
go


CREATE trigger ti_TaxRuleSetRule_MTV on dbo.TaxRuleSetRule AFTER INSERT as
begin

-----------------------------------------------------------------------------------------------------------------------------
-- Name:       ti_TaxRuleSetRule_MTV
-- Arguments: 
-- Tables:    
-- Indexes:   
-- SPs:        
-- Overview:   This custom insert trigger will write records to the custom MTV_TaxAudit table to maintain
--             an easily identifiable record of inserted TaxRuleSetRule records
-- Created by: Eric Wallerstein
-- History:    06/14/2012 - First Created
--*************************************************************************************
--
-- Date Modified  Modified By      Issue# Modification
-- -------------  ---------------  ------ ---------------------------------------------------------------------------------------------
--  06/14/2012    E.Wallerstein           Because SPID information can erroneously remain (or be removed) from the
...
-- Populate the #TRSR temp table with all of the inserted records so that we can attach an Idnty column to each record
...
While @i_Idnty IS NOT NULL
  Begin
    ...
    -- Get the new RuleValue
    Select @vc_NewRuleValue = IsNull(#TRSR.RuleValue,'')
    From   #TRSR
    Where  #TRSR.Idnty = @i_Idnty
    ...
    if @vc_DataObject = 'n_dao_taxrulesetrule_sql' or @vc_TaxRule in ('Destination Location does not have a TCN'
      ...
      Begin
        -- Tax rules based on the n_dao_taxrulesetrule_sql dataobject will be processed here.  We also listed specific
        -- TaxRule descriptions in case the DataObject field isn't used in the future.
    ...
    if @vc_TaxRule = 'Movement Rule'
      Begin
        -- Strip out quotes from the new RuleValue
        Select @vc_NewRuleValue = REPLACE( @vc_NewRuleValue, '"', '')
...
IF OBJECT_ID('dbo.ti_TaxRuleSetRule_MTV') IS NOT NULL
  PRINT '<<< CREATED TRIGGER dbo.ti_TaxRuleSetRule_MTV >>>'
ELSE
  PRINT '<<< FAILED CREATING TRIGGER dbo.ti_TaxRuleSetRule_MTV >>>'
