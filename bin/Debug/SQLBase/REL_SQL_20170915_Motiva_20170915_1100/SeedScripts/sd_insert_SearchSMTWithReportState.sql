...
Print 'Start Script=sd_insert_SearchSMTWithReportState.sql  Domain=MTV  Time=' + Convert(varchar(50), getdate(), 109) + ' on ' + @@SERVERNAME+'.'+db_name()
GO
--delete from reportconfig where rprtcnfgnme = 'Custom Search SMT With Report State'
if not exists ( select 'x' from ReportConfig where RprtCnfgNme = 'Custom Search SMT With Report State' )
 begin

 exec sp_InsertReport 	@vc_foldername	= 'Search Reports',	/* The folder must already exist 		*/
					@vc_reportname	= 'Custom Search SMT With Report State', 	/* The name of the report 			*/
					@vc_reportdescription = 'Search SMT With Report State', 	/* The description of the report 		*/
					@vc_report_dataobject = 'd_mtv_searchsmtwithreportstate',	/* The name of the datawindow <d_?> 		*/
					@vc_report_criteria_object = 'u_criteria_search_smtwithreportstate',	/* The name of the criteria object <u_?> 	*/
					@vc_report_display_object = 'u_search',	/* The name of the display object <u_search_?> 	*/
					@vc_uniqueregistryvalue 	=	'SMTState', 	/* This is a unique name for your report so it can be referenced by name later */
					...
					/* Entity Options */
					@vc_entityname 				 = 'TransactionHeader',  	/* Entity.EnttyNme - This must already exist.  If None just use empty string ''	*/
   ...
   print '<<< New Report Search SMT With Report State into ReportConfig >>>'
end
else
   print '<<< New Report Search SMT With Report State  already exists >>>'
  
  declare @RprtCnfgID int
   
   select @RprtCnfgID = ( select rprtcnfgid from reportconfig where rprtcnfgnme = 'Custom Search SMT With Report State')
