PRINT 'Start Script=sp_MTV_Search_SupplyToFSM_Secondary.sql  Domain=CompanyName  Time=' + CONVERT(VARCHAR(50), GETDATE(), 109) + ' on ' + @@SERVERNAME+'.'+db_name()
GO

IF  OBJECT_ID(N'[dbo].[MTV_Search_SupplyToFSM_Secondary]') IS NULL
      BEGIN
			EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [dbo].[MTV_Search_SupplyToFSM_Secondary] AS SELECT 1'
			PRINT '<<< CREATED StoredProcedure MTV_Search_SupplyToFSM_Secondary >>>'
	  END
GO
...
ALTER PROCEDURE [dbo].[MTV_Search_SupplyToFSM_Secondary]
								(
								@sdt_DateTime			SmallDateTime = Null,
								@vc_ProvisionRuleIDs	Varchar(8000)	= NULL,
								@vc_LocaleIds			Varchar(8000)	= NULL,
								@vc_ProductIds			Varchar(8000)	= NULL,
								@vc_TransferPriceTypes	Varchar(8000)	= NULL,
...
-- Description:   FSM Transfer Price Report
-- ====================================================
-- Date         Modified By     Issue#  Modification
-- -----------  --------------  ------  ---------------------------------------------------------------------
--11/18/2016	JMcClean		4029	Enhancement to add add from and to internal BAs to allow the report to filter by BA
--------------------------------------------------------------------------------------------------------------


If @sdt_DateTime is Null select @sdt_DateTime = getdate()
...
--DECLARE @i_FromBAID Int, @i_ToBAID int 
...
ELSE
	...
	--	FROM BusinessAssociate AS ba 
	--	WHERE ba.BAStts = 'A' AND
	--	ba.BAID = Convert(int, @i_FromBAId)

	--	INSERT #BAIdsTo(ID)
	--	SELECT BAID
	--	FROM BusinessAssociate AS ba 
	--	WHERE ba.BAStts = 'A' AND
	--	ba.BAID = Convert(int, @i_ToBAId)
	--END
...
Begin
	...
	Where	DynLstBxQlfr = 'TransferPriceType' AND DynLstBxTblTyp NOT IN ('YT01')
End
Else
Begin
	Insert	#TransferPriceTypes
...
From	#ProvisionRuleProdLocs
		...
		Inner Join dbo.DealDetailProvision		On	DealDetailProvision.DlDtlPrvsnID = ProvisionRuleDlDtlPrvsn.DlDtlPrvsnID
												...
												And	DealDetailProvision.CostType = 'S'
...
Update	#DistinctPrices
Set		RwPrceDtlID = p.RwPrceDtlID
		,Price = p.RPVle
From	#DistinctPrices
CROSS apply ( select * From dbo.Motiva_FN_GetRawPriceValue(#DistinctPrices.RwPrceLcleID, #DistinctPrices.PriceTypeID,@sdt_DateTime,'A', 0) )p
WHERE p.RwPrceLcleID = #DistinctPrices.RwPrceLcleID AND  #DistinctPrices.TransferPriceType NOT IN ('YT10')

--YT10 prices are based on platts curves for which prices will be as of yesterday hence 
Update	#DistinctPrices
Set		RwPrceDtlID = p.RwPrceDtlID
		,Price = p.RPVle
From	#DistinctPrices
CROSS apply (SELECT * FROM dbo.Motiva_FN_GetRawPriceValue(#DistinctPrices.RwPrceLcleID, #DistinctPrices.PriceTypeID, DateAdd(DAY, -1, @sdt_DateTime), 'A', 1)) p
WHERE p.RwPrceLcleID = #DistinctPrices.RwPrceLcleID AND #DistinctPrices.TransferPriceType IN ('YT10')

SELECT #DealDetailProvisionRows.ProvisionName,	
...
IF  OBJECT_ID(N'[dbo].[MTV_Search_SupplyToFSM_Secondary]') IS NOT NULL
      BEGIN
			EXECUTE	sp_MotivaBuildStatisticsInsertUpdateSQLScripts 'sp_MTV_Search_SupplyToFSM_Secondary.sql'
			PRINT '<<< ALTERED StoredProcedure MTV_Search_SupplyToFSM_Secondary >>>'
	  END
	  ELSE
	  BEGIN
			PRINT '<<< FAILED CREATE OR ALTER on StoredProcedure MTV_Search_SupplyToFSM_Secondary >>>'
	  END    
GO	  


--EXEC [MTV_Search_SupplyToFSM_Secondary]
