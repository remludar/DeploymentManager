If OBJECT_ID('Custom_Closing_Forwards_Report') Is Not NULL
Begin
    DROP PROC Custom_Closing_Forwards_Report
    PRINT '<<< DROPPED PROC Custom_Closing_Forwards_Report>>>'
End
GO
Create Procedure dbo.Custom_Closing_Forwards_Report				@i_P_EODSnpShtID	int				= Null
																,@vc_InternalBAID	varchar(255)	= NULL
																,@vc_DealNumber		varchar(20)		= NULL
																,@vc_StrtgyID		varchar(255)	= NULL
																,@c_OnlyShowSQL		char(1)			= 'N'
															
As	
-----------------------------------------------------------------------------------------------------------------------------
-- Name:			Custom_Closing_Forwards_Report @i_P_EODSnpShtID = 144, @c_OnlyShowSQL = 'Y'  Copyright 2003 SolArc
-- Overview:	
--		
-- Arguments:	
-- SPs:
...
Declare	@vc_DynamicSQL					varchar(max)
		...
		,@sdt_FWDAccntngPrdEndDte		smalldatetime
		,@i_AccntngPrdID				int
...
----------------------------------------------------------------------------------------------------------------------------------
-- Help the report default
----------------------------------------------------------------------------------------------------------------------------------
If	IsNull(@i_P_EODSnpShtID,-1)	= -1
	Select	@i_P_EODSnpShtID	= 0
	
----------------------------------------------------------------------------------------------------------------------------------
-- Set the snapshot control variables
----------------------------------------------------------------------------------------------------------------------------------		
Select	 @i_LastAcctDtlID				= LastAcctDtlID
		,@i_StartedFromVETradePeriodID	= StartedFromVETradePeriodID
...
----------------------------------------------------------------------------------------------------------------------------------
-- Set the 3 month forward end date
----------------------------------------------------------------------------------------------------------------------------------
Select	@sdt_FWDAccntngPrdEndDte	= AccntngPrdEndDte
From	AccountingPeriod	(NoLock)
Where	AccntngPrdID	= @i_AccntngPrdID	+ 3
...
If	@c_OnlyShowSQL = 'Y'
	Begin
		...
		Create Table	#GLResults
						...
						,MarketRwPrceLcleIDText		Varchar(500)	Null 
...
----------------------------------------------------------------------------------------------------------------------
-- Determine the MarketCurveID for the Market Curve
---------------------------------------------------------------------------------------------------------------------- 
Select 	@vc_DynamicSQL = '
Update	#RiskResults
Set		MarketCurveID	= RiskCurve. RwPrceLcleID
From	#RiskResults							(NoLock)
		Inner	Join	RiskPhysicalExposure	(NoLock)	On	#RiskResults. RiskID				= RiskPhysicalExposure. RiskID
		Inner	Join	RiskCurve				(NoLock)	On	RiskPhysicalExposure. RiskCurveID	= RiskCurve. RiskCurveID
Where	#RiskResults. InstanceDateTime						Between RiskPhysicalExposure. StartDate And RiskPhysicalExposure. EndDate		
'
...
Select	#RiskResults. Snapshot							-- P_EODSnpShtID
		...
		,RiskDealDetailProvision. DlDtlPrvsnPrvsnID		-- PrvsnID
		...
		,#RiskResults. PricedInPercentage				-- PricedInPercentage
...
Select 	@vc_DynamicSQL = '
Update	#GLResults
Set		GLTrnsctnTypID	= TrnsctnTypID
		,RecordSource	= ''Accrual''
Where	IsAccrual	= 1
'	
...
----------------------------------------------------------------------------------------------------------------------------------
-- Set the transaction type to either an asset or a loss
----------------------------------------------------------------------------------------------------------------------------------	
Select 	@vc_DynamicSQL = '
Update	#GLResults
Set		GLTrnsctnTypID	=	Case
...
Update	#GLResults
Set		GLTrnsctnTypID	=	Case
								When	ProfitLoss	< 0	Then	2134	-- Unrealized Earnings - Derivative Liability 
								Else	2133							-- Unrealized Earnings - Derivative Asset  	
							End
Where	#GLResults. IsInventory		= 0
And		#GLResults. IsAccrual		= 0	
...
Update	#GLResults
Set		DlDtlPrvsnMntryDrctn	=	Case
										When	GLTrnsctnTypID In (2029)	Then	''R''
										Else	''D''
									End		
Where	#GLResults. IsInventory	= 0
And		#GLResults. IsAccrual	= 0
...
Where	#GLResults. IsInventory		<> 1
And		#GLResults. IsAccrual		= 0
And		#GLResults. GLTrnsctnTypID	In (2027)
...
Where	#GLResults. IsInventory		<> 1
And		#GLResults. IsAccrual		= 0
And		#GLResults. GLTrnsctnTypID	In (2029)
And		#GLResults. ProfitLoss		> 0	
'
...
----------------------------------------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------------------------------------
-- Update the Deal Detail Information
----------------------------------------------------------------------------------------------------------------------------------
Select 	@vc_DynamicSQL = '	
Update	#GLResults
Set		DlDtlINCO	= DynamicListBox. DynLstBxAbbv
From	#GLResults			(NoLock)
		Inner Join	DealDetail		(NoLock)	On	#GLResults. DlHdrID				= DealDetail. DlDtlDlHdrID
												And	#GLResults. DlDtlID				= DealDetail. DlDtlID
...
----------------------------------------------------------------------------------------------------------------------------------
-- Denominalise MarketRwPrceLcleID
----------------------------------------------------------------------------------------------------------------------------------
Select 	@vc_DynamicSQL = '	
Update #GLResults	
Set		MarketRwPrceLcleIDText = CurveName
From	#GLResults					(NoLock)	
		Inner Join	RawPriceLocale	(NoLock)	On #GLResults.MarketRwPrceLcleID	= RawPriceLocale.RwPrceLcleID
'

If @c_OnlyShowSQL = 'Y' Select @vc_DynamicSQL Else Execute (@vc_DynamicSQL)	

----------------------------------------------------------------------------------------------------------------------------------
-- Reset the Record Source for Accruals
----------------------------------------------------------------------------------------------------------------------------------
Select 	@vc_DynamicSQL = '	
Update	#GLResults	
Set		RecordSource	= ''Accruals''
...
Select	P_EODSnpShtID	
		...
		,GL. TrnsctnTypDesc as GL_TrnsctnTypDesc
		,PlnndTrnsfrID
		,StrtgyID
		,Volume	
		,PricedInPercentage * 100
		,TotalValuePerUnit
		,TotalValue
		,TotalValueCrrncyID	
		,SpotTradeFXRate
		,MarketRwPrceLcleIDText
		,MarketValuePerUnit	
		,MarketValue
		,MarketValueCrrncyID
		,ProfitLoss	
		...
		,Energy		
From	#GLResults							(NoLock)
		Left Outer Join	TransactionType		(NoLock)	On	#GLResults. TrnsctnTypID	= TransactionType. TrnsctnTypID
		Left Outer Join	TransactionType	GL	(NoLock)	On	#GLResults. GLTrnsctnTypID	= GL. TrnsctnTypID	
Where	1=1 
'	+	Case
			When	@vc_InternalBAID Is Not Null	Then	'And	#GLResults. InternalBAID	In (' + @vc_InternalBAID + ')
'  	
			Else	''
		End	+	
		Case		
			When	@vc_StrtgyID Is Not Null		Then	'And	#GLResults. StrtgyID		In (' + @vc_StrtgyID + ')
'				   
			Else	''
		End	+
		Case
			When	@vc_DealNumber Is Not Null		Then	'And	#GLResults. DlHdrIntrnlNbr	Like ''' + @vc_DealNumber + '''
'	
			Else	''
		End	+	'
'		
...
If OBJECT_ID('Custom_Closing_Forwards_Report') Is NOT Null
BEGIN
	PRINT '<<< CREATED PROC Custom_Closing_Forwards_Report>>>'
	Grant Execute on Custom_Closing_Forwards_Report to SYSUSER
	Grant Execute on Custom_Closing_Forwards_Report to RightAngleAccess
END
ELSE
	Print '<<<Failed Creating Procedure Custom_Closing_Forwards_Report>>>'
GO
